...
keywordHandler:addKeyword({'eyes'}, StdModule.say, {npcHandler = npcHandler, text = "You pitiful two-eyed creatures. In our eyes, you look weird. It is pathetic how much you depend on hands and legs. Since we beholders have more eyes than any creature in the world, it is obvious that you can determine the value of a species by the number of its eyes. "})
keywordHandler:addKeyword({'beholder'}, StdModule.say, {npcHandler = npcHandler, text = "Our race is very old. Over the time, we have been given many different names by other races. The term beholder sticks to us for quite a while now. In our language the name of our race is not fix but a complex formula, and as such it always changes for the subjective viewer. "})
